    ...
    try:
        ...
        for flight in request.flights:
            ...
            if not isinstance(flight.MES, int) or not (1 <= flight.MES <= 12):
                raise fastapi.HTTPException(status_code=400, detail="MES must be an integer between 1 and 12")
            if flight.TIPOVUELO not in ['I', 'N']:
                raise fastapi.HTTPException(status_code=400, detail='TIPOVUELO must be "I" (International) or "N" (National)')
        ...
        raise fastapi.HTTPException(status_code=500, detail=f"Prediction error: {str(e)}")